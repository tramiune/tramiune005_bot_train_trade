...
def test_sol_rr3():
    print("Fetching 4 years of 15m data for SOL/USDT...")
    since = datetime.now(timezone.utc) - timedelta(days=1460)
    df = _fetch_ohlcv_ccxt("binance", "SOL/USDT", "15m", since=since, limit=200000)
    ...
    while i < len(df) - 1:
        ...
        recent_squeeze = is_squeeze.iloc[i-5:i].any()
        vol_mult = v / v_ma if v_ma > 0 else 0
        
        if recent_squeeze and c > b_up and vol_mult > 2.5 and c > e2:
            entry = c
            sl = entry - 1.5 * at
    ...
    net_r = (wins * 3.0) - losses
    
    print("\n--- SOLANA 15M SQUEEZE (RR = 3.0) ---")
