...
def run_analysis():
    ...
    while i < len(df) - 1:
        ...
        # NO PINBAR REQUIREMENT
        is_proper_session = 8 <= dt.hour <= 18
        is_proper_size = candle_size_pct < 2.5
        
        if is_uptrend and is_proper_speed and is_touching and is_proper_session and is_proper_size:
            entry = c
            ...
            if risk > 0:
                ...
                if exit_idx is not None:
                    final_trades.append(result)
                    i = exit_idx
                    continue
        i += 1
        
    wins = final_trades.count('WIN')
    losses = final_trades.count('LOSS')
    total = len(final_trades)
    print(f"DOGE (No Pinbar + God Mode Filters) -> Total: {total} | Wins: {wins} | Losses: {losses} | WR: {wins/total*100:.1f}% | Net Profit: {(wins * 15.0) - losses} R")
