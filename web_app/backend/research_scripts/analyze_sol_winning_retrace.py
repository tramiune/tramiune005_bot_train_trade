        ...
        if is_uptrend and is_proper_speed and is_touching and is_strong_rejection and has_volume and btc_bullish and is_midweek:
            ...
            if risk > 0:
                ...
                while j < len(df):
                    ...
                    # Track lowest point after hitting +3R
                    if reached_3r:
                        current_low_r = (lo - entry) / risk
                        if current_low_r < lowest_r_after_3r:
                            lowest_r_after_3r = current_low_r
                    
                    # Check if it hits 3R
                    if not reached_3r and hi >= (entry + 3.0 * risk):
                        reached_3r = True
                ...
                if exit_idx is not None:
                    if result == "WIN":
                        winning_trades_retrace.append({
                            ...
                            'lowest_r_after_3r': lowest_r_after_3r if reached_3r else "Never hit 3R (Impossible for a 15R win)"
                        })
                    i = exit_idx
                    continue
        i += 1
        
    print(f"\n--- WINNING TRADES RETRACE ANALYSIS (+3.0R Breakeven Check) ---")
    killed_wins = 0
    for idx, win in enumerate(winning_trades_retrace):
        lowest = win['lowest_r_after_3r']
        if isinstance(lowest, float):
            status = "KILLED BY BREAKEVEN (Tragic!)" if lowest <= 0 else "SURVIVED"
            if lowest <= 0: killed_wins += 1
            print(f"Win {idx+1} ({win['date']}): Dropped to {lowest:+.2f} R after hitting 3R. -> {status}")
            
    print(f"\nTotal Winning Trades: {len(winning_trades_retrace)}")
    print(f"Number of 15R Wins we would have accidentally KILLED: {killed_wins}")
