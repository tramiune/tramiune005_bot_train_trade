...
def fetch_data(symbol, days=1460): # 4 years
...
def run_test_on_symbol(symbol, btc_df):
    ...
    print(f"| {symbol:<10} | Trades: {total:>2} | Wins: {wins:>2} | Losses: {losses:>2} | Winrate: {wr:>4.1f}% | Net Profit: {net_r:>+5.1f} R |")
...
def main():
    ...
    symbols = ["ETH/USDT", "BNB/USDT", "ADA/USDT", "DOGE/USDT", "XRP/USDT"]
    print("\n" + "-"*85)
    print("  TESTING SOLANA GOD MODE ENGINE ON LEGACY ALTCOINS (4 YEARS FUTURES DATA)")
    print("-" * 85)
    for sym in symbols:
        run_test_on_symbol(sym, btc)
    print("-" * 85)
