...
def run_analysis():
    ...
    while i < len(df) - 1:
        ...
        # New "God Mode" Filters
        is_proper_session = 8 <= dt.hour <= 18
        is_proper_size = candle_size_pct < 2.5
        
        if is_uptrend and is_proper_speed and is_touching and is_strong_rejection and has_volume and btc_bullish and is_midweek and is_proper_session and is_proper_size:
            entry = c
            ...
            if risk > 0:
                ...
                if exit_idx is not None:
                    final_trades.append({
                        'date': dt.strftime('%Y-%m-%d %H:00 UTC'),
                        'sol_price': entry,
                        'btc_price': btc_c,
                        'vol': f"{v/v_ma:.1f}x",
                        'result': result,
                        'rr_gained': "+15 R" if result == "WIN" else "-1 R"
                    })
                    i = exit_idx
                    continue
        i += 1
    ...
    print("\n" + "="*70)
    print("   SOLANA RR15 (GOD MODE) - CHRONOLOGICAL TRADE LOG")
    print("="*70)
    
    cap = 10_000_000
    risk = 0.10
    
    for idx, t in enumerate(final_trades):
        print(f"Trade {idx+1:>2} | {t['date']} | SOL: ${t['sol_price']:>6.2f} | BTC: ${t['btc_price']:>6.0f} | Vol: {t['vol']} | {t['result']} ({t['rr_gained']})")
        if t['result'] == "WIN":
            cap += cap * risk * 15.0
        else:
            cap -= cap * risk
    ...
    print("="*70)
    wins = len([t for t in final_trades if t['result'] == 'WIN'])
    losses = len([t for t in final_trades if t['result'] == 'LOSS'])
    print(f"Total: {len(final_trades)} Trades | {wins} Wins | {losses} Losses | Winrate: {wins/len(final_trades)*100:.1f}%")
    print(f"Final Capital (10% Risk): {cap:,.0f} VND")
