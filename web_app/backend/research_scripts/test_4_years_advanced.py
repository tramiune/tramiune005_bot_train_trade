from datetime import datetime, timedelta, timezone
import pandas as pd
import numpy as np
from server.main import _fetch_ohlcv_ccxt, _sma, _rolling_low, _rolling_high, _ema, _atr, _adx

def _rsi(s, n=14):
    delta = s.diff()
    gain = (delta.where(delta > 0, 0)).rolling(window=n).mean()
    loss = (-delta.where(delta < 0, 0)).rolling(window=n).mean()
    rs = gain / loss
    return 100 - (100 / (1 + rs))

def test_4_years():
    print("Fetching 4 years of 1h data...")
    since = datetime.now(timezone.utc) - timedelta(days=1460)
    df = _fetch_ohlcv_ccxt("binance", "BTC/USDT", "1h", since=since, limit=50000)
    print(f"Loaded {len(df)} 1h candles.")

    fast = 9
    slow = 21
    rr = 2.0
    
    close = df["close"].astype(float)
    highs = df["high"].astype(float)
    lows = df["low"].astype(float)
    vol = df["volume"].astype(float)
    
    s_fast = _sma(close, fast)
    s_slow = _sma(close, slow)
    e200 = _ema(close, 200)
    v20 = _sma(vol, 20)

    cross_up = (s_fast > s_slow) & (s_fast.shift(1) <= s_slow.shift(1))
    cross_dn = (s_fast < s_slow) & (s_fast.shift(1) >= s_slow.shift(1))

    atr = _atr(df, 14)
    rsi = _rsi(close, 14)
    adx = _adx(df, 14)

    sl_lookback = 10
    sl_low_base = _rolling_low(lows, sl_lookback).shift(1)
    sl_high_base = _rolling_high(highs, sl_lookback).shift(1)

    trades = []
    i = max(fast, slow, 200, 28) + 2
    
    while i < len(df) - 1:
        entry = float(close.iloc[i])
        r = float(rsi.iloc[i]) if pd.notna(rsi.iloc[i]) else 0
        at = float(atr.iloc[i]) if pd.notna(atr.iloc[i]) else 0
        a = float(adx.iloc[i]) if pd.notna(adx.iloc[i]) else 0
        e2 = float(e200.iloc[i])
        v = float(vol.iloc[i])
        v_ma = float(v20.iloc[i]) if pd.notna(v20.iloc[i]) else 0
        
        side = None
        
        # Long filter: Cross up + Above EMA200 + RSI between 50 and 70 (bullish but not overbought) + ADX > 20 + Vol spike
        if bool(cross_up.iloc[i]) and entry > e2 and 50 < r < 70 and a > 20 and v > v_ma:
            side = "LONG"
            b = float(sl_low_base.iloc[i]) if pd.notna(sl_low_base.iloc[i]) else None
            if b: sl = b - at * 1.5
            
        # Short filter: Cross down + Below EMA200 + RSI between 30 and 50 (bearish but not oversold) + ADX > 20 + Vol spike
        elif bool(cross_dn.iloc[i]) and entry < e2 and 30 < r < 50 and a > 20 and v > v_ma:
            side = "SHORT"
            b = float(sl_high_base.iloc[i]) if pd.notna(sl_high_base.iloc[i]) else None
            if b: sl = b + at * 1.5

        if side:
            risk = abs(entry - sl)
            if risk > 0:
                tp = entry + rr * risk if side == "LONG" else entry - rr * risk
                j = i + 1
                exit_idx = None
                result = None
                
                while j < len(df):
                    hi = float(highs.iloc[j])
                    lo = float(lows.iloc[j])
                    
                    if side == "LONG":
                        if lo <= sl: exit_idx, result = j, "LOSS"; break
                        if hi >= tp: exit_idx, result = j, "WIN"; break
                    else:
                        if hi >= sl: exit_idx, result = j, "LOSS"; break
                        if lo <= tp: exit_idx, result = j, "WIN"; break
                    j += 1
                    
                if exit_idx is not None:
                    trades.append({"side": side, "result": result})
                    i = exit_idx
        i += 1
        
    wins = [t for t in trades if t["result"] == "WIN"]
    losses = [t for t in trades if t["result"] == "LOSS"]
    total = len(trades)
    wr = len(wins) / total * 100 if total else 0
    
    longs = [t for t in trades if t["side"] == "LONG"]
    shorts = [t for t in trades if t["side"] == "SHORT"]
    
    w_longs = len([t for t in longs if t["result"] == "WIN"])
    w_shorts = len([t for t in shorts if t["result"] == "WIN"])
    
    wr_longs = w_longs / len(longs) * 100 if longs else 0
    wr_shorts = w_shorts / len(shorts) * 100 if shorts else 0
    
    print("\n--- ADVANCED 4 YEAR BACKTEST RESULTS ---")
    print(f"Total Trades: {total} (Wins: {len(wins)}, Losses: {len(losses)})")
    print(f"Overall Winrate: {wr:.2f}% (RR = {rr})")
    print(f"Long Trades: {len(longs)} (Winrate: {wr_longs:.2f}%)")
    print(f"Short Trades: {len(shorts)} (Winrate: {wr_shorts:.2f}%)")

if __name__ == "__main__":
    test_4_years()
