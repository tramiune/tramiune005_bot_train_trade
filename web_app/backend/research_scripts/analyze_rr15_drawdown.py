...
def run_analysis():
    ...
    print(f"\nAnalyzed {len(trades)} Trades Chronologically:\n")
    
    balance_r = 0.0
    for t in trades:
        res = t["result"]
        date_str = t["entry_time"].strftime("%Y-%m")
        if res == "LOSS":
            current_losses += 1
            current_streak_dates.append(date_str)
            balance_r -= 1.0
            print(f"[{date_str}] LOSS (-1R) | Balance: {balance_r:+.1f}R")
        else:
            if current_losses > max_losses:
                max_losses = current_losses
                loss_streak_dates = list(current_streak_dates)
            current_losses = 0
            current_streak_dates = []
            balance_r += 15.0
            print(f"[{date_str}] WIN (+15R) | Balance: {balance_r:+.1f}R  <<< BOOM!")
    ...
    print(f"Max Loss Streak Dates: {', '.join(loss_streak_dates)}")
