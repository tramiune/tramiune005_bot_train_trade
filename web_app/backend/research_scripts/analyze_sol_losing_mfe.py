    ...
    while i < len(df) - 1:
        ...
        if is_uptrend and is_proper_speed and is_touching and is_strong_rejection and has_volume and btc_bullish and is_midweek:
            ...
            if risk > 0:
                ...
                exit_idx = None
                result = None
                max_high_reached = entry
                
                while j < len(df):
                    hi = float(df["high"].iloc[j])
                    lo = float(df["low"].iloc[j])
                    
                    if hi > max_high_reached:
                        max_high_reached = hi
                ...
                if exit_idx is not None:
                    if result == "LOSS":
                        max_r_reached = (max_high_reached - entry) / risk
                        losing_trades_mfe.append({
                    ...
                    continue
        i += 1
    return losing_trades_mfe

def run():
    losses = get_futures_sequence()
    print(f"\n--- MAXIMUM FAVORABLE EXCURSION (MFE) OF THE {len(losses)} LOSING TRADES ---")
    for idx, loss in enumerate(losses):
        print(f"Loss {idx+1} ({loss['date']}): Pumped to +{loss['max_r']:.2f} R before dumping to SL")
        
    above_2r = len([l for l in losses if l['max_r'] >= 2.0])
    above_5r = len([l for l in losses if l['max_r'] >= 5.0])
    print(f"\nSummary:")
    print(f"Trades that pumped > +2.0R before dying: {above_2r}")
    print(f"Trades that pumped > +5.0R before dying: {above_5r}")
