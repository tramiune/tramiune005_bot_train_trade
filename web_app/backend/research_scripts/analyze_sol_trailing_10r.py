            ...
            if risk > 0:
                ...
                while j < len(df):
                    ...
                    if reached_10r:
                        ...
                        if current_low_r < lowest_r_after_10r:
                            lowest_r_after_10r = current_low_r
                            
                    if not reached_10r and hi >= (entry + 10.0 * risk):
                        reached_10r = True
                ...
                if exit_idx is not None:
                    if result == "WIN":
                        winning_trades_retrace.append({
                            ...
                            'lowest_r_after_10r': lowest_r_after_10r if reached_10r else "Never hit 10R"
                        })
                    i = exit_idx
                    continue
        i += 1
        
    print(f"\n--- WINNING TRADES RETRACE ANALYSIS (Reach +10R, trail to +7R) ---")
    killed_wins = 0
    for idx, win in enumerate(winning_trades_retrace):
        lowest = win['lowest_r_after_10r']
        if isinstance(lowest, float):
            status = "KILLED BY +7R TRAIL" if lowest <= 7.0 else "SURVIVED (Hit 15R perfectly)"
            if lowest <= 7.0: killed_wins += 1
            print(f"Win {idx+1} ({win['date']}): Dropped to {lowest:+.2f} R after hitting 10R. -> {status}")
