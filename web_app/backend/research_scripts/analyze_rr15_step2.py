...
def run_analysis():
    ...
    df["bbw"] = (std20 * 4) / sma20 * 100 # BB Width in percentage
    ...
    while i < len(df) - 1:
        ...
        if is_uptrend and is_proper_speed and is_touching and is_strong_rejection and has_volume and btc_bullish:
            entry = c
            sl = entry - 1.5 * at
    ...
    print(f"\nAnalyzed {len(trades)} BTC-Filtered trades for RR 15.0 (Wins: {len(wins)}, Losses: {len(losses)})")
    
    print("\n--- 1. RSI(14) HIDDEN DIVERGENCE (Is it too weak?) ---")
    for r in [(0, 40), (40, 50), (50, 60), (60, 100)]:
        w = len([t for t in wins if r[0] <= t["rsi"] < r[1]])
        l = len([t for t in losses if r[0] <= t["rsi"] < r[1]])
        tot = w + l
        if tot > 0: print(f"RSI {r[0]}-{r[1]}: {tot:>2} trades -> Wins: {w}, Losses: {l} (WR: {w/tot*100:.1f}%)")
        
    print("\n--- 2. BOLLINGER BAND WIDTH (Volatility at Retest) ---")
    for r in [(0, 5), (5, 10), (10, 20), (20, 100)]:
        w = len([t for t in wins if r[0] <= t["bbw"] < r[1]])
        l = len([t for t in losses if r[0] <= t["bbw"] < r[1]])
        tot = w + l
        if tot > 0: print(f"BBW {r[0]}-{r[1]}%: {tot:>2} trades -> Wins: {w}, Losses: {l} (WR: {w/tot*100:.1f}%)")
        
    print("\n--- 3. DAY OF WEEK ---")
    for d, d_name in enumerate(["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]):
        w = len([t for t in wins if t["day"] == d])
        l = len([t for t in losses if t["day"] == d])
        tot = w + l
        if tot > 0: print(f"{d_name}: {tot:>2} trades -> Wins: {w}, Losses: {l}")
