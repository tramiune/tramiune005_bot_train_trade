...
def get_futures_sequence():
    ...
    # Pre-fetched data optimization (mocking fetch for speed, we already know the sequence logic)
    # Actually I need the real sequence, let's fetch fast.
    all_sol = []
    sol_since = since
    while True:
        try:
...
def run_simulation():
    trades = get_futures_sequence()
    print(f"Trade Sequence ({len(trades)} trades): {trades}")
    ...
    print(f"\nFINAL CAPITAL (15% RISK): {cap:,.0f} VND")
    print(f"Max Drawdown: -{max_dd:.2f}%")
    print(f"Lowest Capital Hit: {min_cap:,.0f} VND")
